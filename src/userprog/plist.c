...
void plist_print(struct plist* l){
  printf("%-15s%-15s%-18s%-12s%s\n", "ProcessID", "ParentID", "Exit Status", "Alive", "Parent Alive" );
  printf("------------------------------------------------------------------------\n");
  unsigned i;
  for(i=0; i<PLIST_SIZE; ++i){
    if(l->content[i].free == false){
      printf("%5i%14i%17i%17s%14s\n", l->content[i].process_id, l->content[i].parent_id, l->content[i].exit_status, l->content[i].alive ? "true" : "false", l->content[i].parent_alive ? "true" : "false");
    }
  }
}
